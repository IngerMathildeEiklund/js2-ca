...
const prevBTN = document.getElementById('prev-button') as HTMLButtonElement | null;
const nextBTN = document.getElementById('next-button') as HTMLButtonElement | null;
let totalPagesP = document.getElementById('total-pages') as HTMLParagraphElement | null;
...
export async function loadPage(page: number): Promise <void> {
    try {
        const response = activeQuery ? await searchForPosts(activeQuery, page, POSTS_PER_PAGE) : await getPosts(page, POSTS_PER_PAGE);
