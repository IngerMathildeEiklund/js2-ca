...
const addOptionalImageBTN = document.getElementById('add-image-button') as HTMLButtonElement;
addOptionalImageBTN.addEventListener('click', (event) => {
    event.preventDefault();
    const optionalImageContainer = document.querySelector('.optional-image-container');
  ...
  const media = formData.imageUrl ? { url: formData.imageUrl, alt: formData.imageAlt || 'No image added.'} : undefined;
  publishPostBTN.disabled = true;

  try {

    await publishNewPost(formData.title, formData.publishPost, formData.tags, media);
